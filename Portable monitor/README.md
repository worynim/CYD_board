...
5. **호스트 GUI 컨트롤 패널 (Tkinter 기반)**
   ...
   - **특정 창(window) 캡처 (macOS)**: 캡처 화면 콤보에서 보이는 앱 창을 선택하면 그 창 영역만 전송하며, 창 이동·리사이즈를 실시간 추적합니다. `🔄` 버튼으로 창 목록을 새로 고칩니다.
...
- **특정 창 캡처 (macOS)**: 창 제목이 목록에 표시되려면 **화면 기록(Screen Recording) 권한**이 필요합니다 — 모니터 캡처에 이미 필요한 권한과 같아 별도 설정은 없습니다. 선택한 창을 닫으면 마지막 위치의 정지 화면이 전송되며, `🔄`로 목록을 새로 고친 뒤 다시 선택하세요. 창이 다른 창에 가려지면 가려진 부분은 그 위의 내용이 캡처됩니다.
